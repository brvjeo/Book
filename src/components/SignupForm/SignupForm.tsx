...
export const SignupForm: React.FC<TProps> = ({onClose: closeModal}): React.ReactElement | null => {
    ...
    const signupHandler = (values: TFormValues) => {
        firebaseApp.createUserWithEmailAndPassword(values.email, values.password)
            .then(
                userCredential => application.createUser(userCredential.user.uid, values.name, values.lastname, values.email, (userRecord, user, resolve, reject) => {
                    firebaseApp.writeDB(DB_ROUTES.userIDS, userRecord).then(_ => resolve(user.id)).catch(reject);
                })
            )
            .then(id => navigate(`/${id}/articles`))
            .catch(e => {
                if(e.message){
                    ...
                    <div className={styles.container}>
                        ...
                        <Form>
                            ...
                            <Input
                                ...
                                size={INPUT_SIZE.L}/>
                            {
                                error.dispatched && <div className={styles.error}>{error.error}</div>
                            }
                            <Button type={'submit'} size={BUTTON_SIZE.M} className={styles.buttonCreate}>Create</Button>
                        </Form>
                    </div>
                )
            }
